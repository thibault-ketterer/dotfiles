...
function! DetectAndCallTree()
    if &filetype == 'yaml'
        call YAMLTree()
    elseif &filetype == 'json'
        call JSONTree()
    else
        echo "Unsupported file type for tree view."
    endif
endfunction

nnoremap <F5> :call DetectAndCallTree()<CR>
...
function! JSONTree()
    ...
    for l:n in reverse(range(1, l:cur))
        let l:i = indent(l:n)
        let l:line = getline(l:n)
        " Extract JSON key, assuming it's in the form of "key": value
        let l:key = substitute(l:line, '^\s*"\([^"]\+\)":.*', "\\1", '')
        " If the indentation decreased and the pattern matched
        if (l:i < l:indent && l:key !=# l:line)
            let l:list = add(l:list, l:key)
            let l:indent = l:i
